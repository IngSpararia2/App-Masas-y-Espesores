...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final visiblePrediction = prediction != null &&
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (selectedSummary != null) ...[
                            const SizedBox(height: 14),
                            _HistoricalRange(summary: selectedSummary),
                          ],
                          if (_localError != null) ...[
  ...
  Future<void> _calculate() async {
    ...
    try {
      await widget.controller.calculate(
        ...
        absorption: _parseNumber(_absorptionController.text)!,
        thicknessMm: _type == MeasurementType.flexure
            ? _parseNumber(_thicknessController.text)
            : null,
      );
    } catch (error) {
      setState(() {
        _localError = error.toString().replaceFirst('Bad state: ', '');
      });
    } finally {
...
class _HistoricalRange extends StatelessWidget {
  const _HistoricalRange({required this.summary});
      ...
      child: Wrap(
        ...
        children: [
          ...
          _MiniFact(
            ...
            value: '${summary.minAbsorption.toStringAsFixed(2)}–'
                '${summary.maxAbsorption.toStringAsFixed(2)} %',
          ),
          if (thickness != null)
            _MiniFact(label: 'Espesor histórico', value: thickness),
        ],
      ),
    );
  }
...
class _ScenarioCard extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _MassLine(
              label: 'Masa seca',
              value: _formatMass(scenario.dryMass, result.type),
              unit: unit,
            ),
            _MassLine(
              label: 'Masa saturada',
              value: _formatMass(scenario.saturatedMass, result.type),
              unit: unit,
            ),
            _MassLine(
              label: 'Masa inmersa',
              value: _formatMass(scenario.immersedMass, result.type),
              unit: unit,
            ),
            if (scenario.naturalMass != null)
              _MassLine(
                label: 'Masa natural',
                value: _formatMass(scenario.naturalMass!, result.type),
                unit: unit,
              ),
            const Divider(height: 20),
            Text('Densidad usada: ${scenario.density.toStringAsFixed(0)} kg/m³'),
          ],
        ),
      ),
    );
  ...
  static String _formatMass(double value, MeasurementType type) {
...
class _MassLine extends StatelessWidget {
  const _MassLine({
    required this.label,
    required this.value,
    required this.unit,
